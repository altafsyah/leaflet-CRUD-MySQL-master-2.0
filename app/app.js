...
promiseCategories.then(function (data) {
  ...
  categories.forEach(e => {
    categoryOptions += `<option value="${e['id']}">${e.category}</options>`
  });
});
...
function _buildDigitiseModalBox(targetmodal, context, geometry) {
  ...
  var htmlformcomponent = "" +
    ...
    "<thead>" +
    "<tr>" +
    "<th colspan='2' class='text-center'>Feature</th>" +
    "</tr>" +
    "</thead>" +
    "<tbody>" +
    "<tr>" +
    ...
    "<td class=''>" +
    "<select name='category' id='category' form='dynamicform'>" +
    categoryOptions +
    "</select>" +
    "</td>" +
    "</tr>" +
...
function _activateFeatureSave() {
  $("#savegeometrydata").on('click', function (evt) {
    ...
    var formValue = document.getElementById('dynamicform');
